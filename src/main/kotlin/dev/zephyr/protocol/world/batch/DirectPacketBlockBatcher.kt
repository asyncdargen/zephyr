...
import dev.zephyr.protocol.NativeProtocol.channel
...
data object DirectPacketBlockBatcher : BlockBatcher {
    ...
    override fun batch(
        ...
        players.firstNotNullOfOrNull { it.channel }?.eventLoop()?.execute {
    ...
    override fun batch(
        ...
        players.firstNotNullOfOrNull { it.channel }?.eventLoop()?.execute {
            ...
            packet.writeVarInt(blocks.filterNotNull().count())
            blocks.forEachIndexed { position, block ->
                block ?: return@forEachIndexed

                packet.writeVarLong((block.stateId shl 12).toLong() or position.toLong())
            }
            players.forEach { it.sendForcePacket(packet) }
        }
    }
